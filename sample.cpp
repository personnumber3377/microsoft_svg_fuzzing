...
/*
void run_with_timeout(const std::vector<uint8_t>& buffer, DWORD timeout_ms) {
    HANDLE hThread = CreateThread(NULL, 0, FuzzThreadFunc, (LPVOID)&buffer, 0, NULL);

    DWORD result = WaitForSingleObject(hThread, timeout_ms);
    if (result == WAIT_TIMEOUT) {
        TerminateThread(hThread, 1);
        FILE* f = fopen("C:\\Users\\elsku\\hook_hit.txt", "a+");
        if (f) {
            fprintf(f, "[!] TIMEOUT - thread killed\n");
            fclose(f);
        }
    } else {
        // completed normally
...
DWORD WINAPI FuzzThreadFunc(LPVOID lpParam) {
    auto* buffer = static_cast<std::vector<uint8_t>*>(lpParam);
...
void poopoothing(char* filename) {
    ...
    */
    ...
    DWORD result = WaitForSingleObject(hThread, 5000); // Wait for 5 seconds....
    if (result == WAIT_TIMEOUT) {
        TerminateThread(hThread, 1);
        FILE* f = fopen("C:\\Users\\elsku\\hook_hit.txt", "a+");
        if (f) {
